...
namespace Croogine {
	...
	void CroogineModel::createVertexBuffers(const std::vector<Vertex>& vertices) {
		...
		vertexBuffer = std::make_unique<CroogineBuffer>(
			croogineDevice,
			vertexSize,
			vertexCount,
		...
		croogineDevice.copyBuffer(stagingBuffer.getBuffer(), vertexBuffer->getBuffer(), bufferSize);
	}
	...
	void CroogineModel::createIndexBuffers(const std::vector<uint32_t>& indices) {
		...
		indexBuffer = std::make_unique<CroogineBuffer>(
			croogineDevice,
			indexSize,
			indexCount,
		...
		croogineDevice.copyBuffer(stagingBuffer.getBuffer(), indexBuffer->getBuffer(), bufferSize);
